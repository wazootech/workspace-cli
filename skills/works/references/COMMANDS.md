# works command reference

The `works` CLI manages a multi-repo Wazoo workspace without Git submodules. It
...
| Command                                                                                     | Purpose                                                                                                                                                                                                                               |
| :------------------------------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `works check`                                                                               | Read-only baseline check. Reports `CLEAN`, `DIRTY`, `FEATURE_CLEAN`, `DIVERGED`, `UNKNOWN`, `MISSING`, `UNMANAGED` states.                                                                                                            |
| `works check --json`                                                                        | Structured per-repo state for tools.                                                                                                                                                                                                  |
| `works init [--host <host>] [--owner <owner>] [<repo...>]`                                  | One-time scaffold for an empty directory: writes `workspace.json` (schema v4) with optional host/owner and seeded shorthand entries, and creates `repos/`, `worktrees/`, `secrets/`. Fails closed if any manifest already exists.     |
| `works install [<repo...>]`                                                                 | Clone missing repos, converging in one invocation: each pass re-resolves the tree, so newly detected sub-workspaces bootstrap without reruns. Scoped targets stay single-pass. Fresh clones lack gitignored files and repo setup.     |
| `works add [<name>] [--url <url>] [--name <n>] [--create] [--visibility <public\|private>]` | Append a manifest entry: shorthand string, or object via `--url`. Surgical edit (comments preserved). GitHub shorthands probed with `gh`; `--create` makes a missing repo first (default private). Never clones; run `install` after. |
| `works remove <repo>`                                                                       | Delete the manifest entry by effective name. Surgical edit; local checkouts are never deleted.                                                                                                                                        |
| `works update`                                                                              | Fetch remotes and fast-forward only clean default branches.                                                                                                                                                                           |
| `works worktree add <repo> <feature> [<commit-ish>]`                                        | Create a git worktree under `worktrees/<repo>/<feature>/`, branching from `origin/<default>` or an explicit `<commit-ish>`.                                                                                                           |
| `works worktree list [--stale] [--json]`                                                    | List worktrees across all repos. `--stale` filters to worktrees whose branch is fully merged (safe removal candidates).                                                                                                               |
| `works worktree remove <repo> <feature>`                                                    | Remove a worktree, then prune and tidy the empty `worktrees/<repo>/` directory.                                                                                                                                                       |
| `works env sync [--dry-run]`                                                                | Copy local environment files from a gitignored `secrets/` directory into checkouts and worktrees.                                                                                                                                     |
| `works validate`                                                                            | Validate the manifest without touching any repository.                                                                                                                                                                                |

## Worktree creation rules

- **Baseline rule**: `works worktree add` branches from `origin/<default>` (via
...
`works check` classifies each repository:
...
A `WORKTREE_DIRTY` or `ERROR` state can also appear. If `works check` exits `1`
...
- **`PATH_BLOCKED` or `INVALID` during `install`**: An existing path occupies
  the expected repository location but is not a Git repository. `works install`
  fails closed without touching it. Remove or relocate the blocking path
  manually.
- **Symlink rejection in `env sync`**: `works env sync` refuses to overwrite any
  destination path that is a symbolic link.
- **Dirty linked worktrees**: `works check` inspects primary checkouts and
