...
`works worktree add` created feature branches with
...
`works worktree list --stale` listed detached worktrees, conflating a detached
HEAD with a worktree that is safe to delete.

## Decision

1. `works worktree add <repo> <feature> [<commit-ish>]` branches from the
   ...
   `works update` remains the refresh step, matching git's own behavior of
...
3. `works worktree list --stale` lists linked worktrees whose branch has no
...
4. After a successful `works worktree remove`, the now-empty `worktrees/<repo>/`
   parent directory is removed (best-effort).
